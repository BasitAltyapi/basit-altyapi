...
(async () => {
  ...
  if (argv.get(0) === "guild") {
    ...
    publishMode = argv.get(0) === "global" ? "global" : null;
  }
    ...
    if (!isClearMode) {
        ...
        dcInters = uInters.reduce((all, current) => {
          Label:if (current.name.length === 1) {
            {
              all.push({
          ...
          } else if (current.name.length === 3) {
            {
              ...
              if (!level1Item) {
                ...
                let level2Item = level1Item.options.find(i => i.name === current.name[1] && i.type === "SUB_COMMAND_GROUP");
                if (!level2Item) {
  ...
  /** @type {import("discord-api-types/rest/v9/user").RESTGetAPIUserResult} */
    const [me] = await Promise.all([rest.get(Routes.user())]);
